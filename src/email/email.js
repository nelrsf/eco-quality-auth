...
function sendRecoveryPasswordEmail(code, email) {
  return new Promise((resolve, reject) => {
    ...
    const message = {
      ...
      html: `Se ha solicitado recuperar la contraseña de la cuenta ${email}, si usted no ha hecho este cambio, por favor comuníquese al correo nel.rsf@gmail.com<br><br>Para recuperar la contraseña haga clic en el siguiente enlace:<br>http://localhost:4300/auth/recovery/${encodedParam} <br>Este enlace de recuperación tiene un tiempo de validez de 5 minutos`,
    };
    // Enviar el mensaje
    transporter.sendMail(message, function (err, info) {
      if (err) {
